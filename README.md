...
在后台文章编辑/页面编辑页搜索国内各大视频网站的影视资源，并一键将搜索的影视以短代码的形式插入到编辑器中，前台文章/页面中可直接播放；
...
插件CSS包含暗色模式，元素标签类名： `body.dark` ，可搭配暗色主题使用。

## 使用方法

下载插件，上传到插件目录，文件夹改名为 `VideoCollector` ，在后台启用插件，进行相关设置（可选），在撰写新文章或页面的编辑器右侧视频采集输入框输入要插入的视频标题，点击搜索视频，选择平台，点击搜索结果的视频标题即可插入视频、分集按钮和视频简介。
...
注意！！！由于采集的第三方视频无版权，使用此插件，可能涉及盗版风险，若存在版权纠纷，与插件开发者无关！请酌情使用。
