...
下载插件，上传到插件目录，在后台启用插件，进行相关设置（可选），在撰写新文章或页面的编辑器右侧视频采集输入框输入要插入的视频标题，点击搜索视频，选择平台，点击搜索结果的视频标题即可插入视频、分集按钮和视频简介。
...
如果你是从旧版升级的，由于1.0.3版本更换了短代码变量名称，请将原文章或页面中的短代码`[hls]`改成`[play]`（开头结尾都要改），否则以前的视频将无法正常播放；
